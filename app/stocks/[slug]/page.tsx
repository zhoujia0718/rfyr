...
export default function StockArticlePage() {
  ...
  return (
    <>
      ...
      {mounted && membershipRequired === true && !quotaDismissed && !requireLogin && (
        <WechatGuideOverlay
          open={true}
...
function buildArticlePage(
  ...
  articles.forEach((a: any) => {
    const categoryName = a.category || "个股挖掘"
  ...
  const breadcrumbs = article
    ? [{ title: "个股挖掘", href: "/stocks" }, { title: article.title?.replace(/\s*[|｜]\s*\S+$/, '') || "文章" }]
    : [{ title: "个股挖掘", href: "/stocks" }]
